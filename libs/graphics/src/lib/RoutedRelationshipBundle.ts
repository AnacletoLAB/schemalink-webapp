...
export class RoutedRelationshipBundle {
  ...
  constructor(
    ...
    const leftRadius = Math.max(
      ...arrowDimensions.map(
        (arrow) =>
          (arrow.leftToRight ? arrow.startRadius : arrow.endRadius) || 0
      )
    );
    const rightRadius = Math.max(
      ...arrowDimensions.map(
        (arrow) =>
          (arrow.leftToRight ? arrow.endRadius : arrow.startRadius) || 0
      )
    );
    ...
    if (relationships[0].startAttachment || relationships[0].endAttachment) {
      ...
      const neighbourAngles = neighbourPositions(selfNode.node, graph).map(
        (position) => position.vectorFrom(selfNode.position).angle()
      );
      const biggestAngularSpace = clockwiseAngularSpace(neighbourAngles);
      const angularSeparation =
        biggestAngularSpace.gap /
        (relationships.length + Math.sign(neighbourAngles.length));
      ...
      const firstDisplacement =
        (-(relationships.length - 1) * relationshipSeparation) / 2;
      const middleRelationshipIndex = (relationships.length - 1) / 2;

      const maxDeflection = Math.PI / 2;
      let leftTightening = 0.6;
      if (
        (relationshipSeparation * (relationships.length - 1) * leftTightening) /
          leftRadius >
        maxDeflection
      ) {
        leftTightening =
          (maxDeflection * leftRadius) /
          (relationshipSeparation * (relationships.length - 1));
      }
      if (
        (leftRadius + maxLeftHeadHeight) *
          Math.sin((leftTightening * -firstDisplacement) / leftRadius) >
        -firstDisplacement
      ) {
        leftTightening =
          (Math.asin(firstDisplacement / (leftRadius + maxLeftHeadHeight)) *
            leftRadius) /
          firstDisplacement;
      }
      ...
      if (
        ...
        rightTightening =
          (maxDeflection * rightRadius) /
          (relationshipSeparation * (relationships.length - 1));
      }
      if (
        (rightRadius + maxRightHeadHeight) *
          Math.sin((rightTightening * -firstDisplacement) / rightRadius) >
        -firstDisplacement
      ) {
        rightTightening =
          (Math.asin(firstDisplacement / (rightRadius + maxRightHeadHeight)) *
            rightRadius) /
          firstDisplacement;
      }
      ...
      for (let i = 0; i < relationships.length; i++) {
        ...
        if (i === middleRelationshipIndex) {
          ...
          const displacement =
            (firstDisplacement + i * relationshipSeparation) *
            (dimensions.leftToRight ? 1 : -1);
          const arrow = new ParallelArrow(
            ...
            dimensions.endRadius || 0,
            displacement *
              (dimensions.leftToRight
                ? leftTightening / leftRadius
                : rightTightening / rightRadius),
            displacement *
              (dimensions.leftToRight
                ? rightTightening / rightRadius
                : leftTightening / leftRadius),
  ...
  boundingBox() {
    return combineBoundingBoxes(
      this.routedRelationships.map((routedRelationship) =>
        routedRelationship.boundingBox()
      )
    );
  }
