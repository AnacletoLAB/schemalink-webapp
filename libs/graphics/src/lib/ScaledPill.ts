...
export class ScaledPill {
  ...
  constructor(
    ...
    // Scale down the border width
    const baseBorderWidth = style('label-border-width') as number;
    this.borderWidth = Math.max(1, baseBorderWidth * 0.5 * scaleFactor);
    
    this.display = style('label-display') as string;

    // Scale down padding
    const basePadding = style('label-padding') as number;
    const padding = Math.max(2, basePadding * 0.6 * scaleFactor);

    // Scale down font size - use provided baseFontSize if available, otherwise use label-font-size
    // For relationship ontologies, baseFontSize should be type-font-size to ensure it's <= relationship name
    const fontSizeBase = baseFontSize !== undefined 
      ? baseFontSize 
      : (style('label-font-size') as number);
    const fontSize = Math.max(8, fontSizeBase * 0.65 * scaleFactor);
  ...
  drawSelectionIndicator(ctx: DrawingContext) {
    const indicatorWidth = Math.max(4, 10 * this.scaleFactor);
    ctx.save();
