...
const allEntitiesSelected = (oldEntities, newEntities) => {
  return newEntities.every((newEntity) =>
    oldEntities.some((oldEntity) => entitiesMatch(oldEntity, newEntity))
  );
};

const entitiesMatch = (entity1, entity2) =>
  entity1.entityType === entity2.entityType && entity1.id === entity2.id;
...
export default function selection(
  ...
  switch (action.type) {
    ...
    case 'TOGGLE_SELECTION': {
      ...
      if (
        state.editing &&
        entities.some((selectedEntity) =>
          entitiesMatch(selectedEntity, state.editing)
        )
      ) {
